...
static CBigNum bnProofOfWorkFirstBlock(~uint256(0) >> 30);
...
const string strMessageMagic = "MammothCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // MammothCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // MammothCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
static const int64_t ifac1 = 249;		
static const int64_t ifac2 = 19;	
...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	...
	int random = generateMTRandom(seed, 72000);

	// printf(">> cseed = %s, random = %d\n", cseed, random);

	if(random > 26960 && random < 26971)		// 1/7200
	{
		return ifac1;
	}
	else if(random > 46300 && random < 46401)	// 1/720
	{
		return ifac2;
	}
...
{
    int64_t nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE_STABLE;
	int nPoSHeight = GetPosHeight(pindex);
	int64_t nSubsidy = 0;

	if(nPoSHeight < YEARLY_POS_BLOCK_COUNT)	
	{
		nSubsidy = 25 * nRewardCoinYear * nCoinAge / 365;
	}
	else if(nPoSHeight < 2 * YEARLY_POS_BLOCK_COUNT)	
	{
		nSubsidy = 12.5 * nRewardCoinYear * nCoinAge / 365;
	}
	else if(nPoSHeight < 3 * YEARLY_POS_BLOCK_COUNT)	
	{
		nSubsidy = 6.5 * nRewardCoinYear * nCoinAge / 365;
	}
	else if(nPoSHeight < 4 * YEARLY_POS_BLOCK_COUNT)	
	{
		nSubsidy = 3.5 * nRewardCoinYear * nCoinAge / 365;
	}
...
// MammothCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // MammothCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // MammothCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // MammothCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // MammothCoin: track money supply and mint amount info
...
// MammothCoin: total coin age spent in transaction, in the unit of coin-days.
...
// MammothCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // MammothCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // MammothCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // MammothCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // MammothCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // MammothCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // MammothCoin: check proof-of-stake
    ...
    // MammothCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // MammothCoin: getblocks may not obtain the ancestor block rejected
    ...
    // MammothCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MammothCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MammothCoin: 1st X13 coin with superblocks!";
        CTransaction txNew;
        txNew.nTime = 1401666344;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401666344;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 6553990;
        if(fTestNet)
        {
            block.nNonce = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("b2af11a4008b4e912cd57c8c99d47e81cf572351fa50bb305a9642b902965b2b"));
        block.print();
        ...
        // MammothCoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xda, 0xef, 0xfa, 0xfd };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // MammothCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // MammothCoin: tell downloading node about the latest block if it's
